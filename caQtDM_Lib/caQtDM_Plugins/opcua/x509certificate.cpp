// The following file is taken from https://github.com/basysKom/opcua_browser/ and modified under the GPL-3
...
bool X509Certificate::createCertificate(const QString &pkiDir)
{
    ...
    // Generate RSA Key
    QOpcUaKeyPair key;
    key.generateRsaKey(QOpcUaKeyPair::RsaKeyStrength::Bits4096);
    ...
    QByteArray keyData;
    {
        ...
        keyData = key.privateKeyToByteArray(QOpcUaKeyPair::Cipher::Aes128Cbc, pemPassword);
    }

    QFile keyFile(privatePath + "/caQtDM.pem");
    ...
    dn.setEntry(QOpcUaX509DistinguishedName::Type::CommonName, "caQtDM");
    dn.setEntry(QOpcUaX509DistinguishedName::Type::CountryName, "CH");
    dn.setEntry(QOpcUaX509DistinguishedName::Type::LocalityName, "Villigen");
    dn.setEntry(QOpcUaX509DistinguishedName::Type::StateOrProvinceName, "Aargau");
    dn.setEntry(QOpcUaX509DistinguishedName::Type::OrganizationName, "Paul Scherrer Institut PSI");
    csr.setSubject(dn);
    ...
    QFile certFile(certsPath  + "/caQtDM.der");
