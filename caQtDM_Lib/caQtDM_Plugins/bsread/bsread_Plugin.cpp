...
Q_LOGGING_CATEGORY(bsread, "caqtdm.plugins.bsread");
...
{
    qCInfo(bsread) << "bsreadPlugin: Create";
...
int bsreadPlugin::initCommunicationLayer(MutexKnobData *data, MessageWindow *messageWindow,QMap<QString, QString> options)
{
    int i;
    //Q_UNUSED(options);
    qCDebug(bsread) << "bsreadPlugin: InitCommunicationLayer" << data;
    qCDebug(bsread) << "bsreadPlugin: InitCommunicationLayer with options" << options;
    ...
    if (DispacherConfig.length()>0){
        ...
        qCDebug(bsread) << "Start Status:" <<DispatcherThread->isFinished();
...
int bsreadPlugin::pvAddMonitor(int index, knobData *kData, int rate, int skip) {
    ...
    int i;
    QMutexLocker locker(&mutex);
    qCDebug(bsread) << "bsreadPlugin:pvAddMonitor" << kData->pv << kData->index << kData;
...
int bsreadPlugin::pvClearMonitor(knobData *kData) {
    ...
    qCDebug(bsread) << "bsreadPlugin:pvClearMonitor" << kData << kData->pv << kData->index <<bsreadconnections.size();
    while (i<bsreadconnections.size()){
...
{
    qCDebug(bsread) << "DemoPlugin:pvFreeAllocatedData";
...
int bsreadPlugin::pvSetValue(char *pv, double rdata, int32_t idata, char *sdata, char *object, char *errmess, int forceType) {
    ...
    Q_UNUSED(object);
    QMutexLocker locker(&mutex);
    qCDebug(bsread) << "bsreadPlugin:pvSetValue" << pv << rdata << idata << sdata;
...
int bsreadPlugin::pvSetWave(char *pv, float *fdata, double *ddata, int16_t *data16, int32_t *data32, char *sdata, int nelm, char *object, char *errmess) {
    ...
    QMutexLocker locker(&mutex);
    qCDebug(bsread) << "bsreadPlugin:pvSetWave";
    return false;
}
...
int bsreadPlugin::pvGetTimeStamp(char *pv, char *timestamp) {
    Q_UNUSED(pv);
    qCDebug(bsread) << "bsreadPlugin:pvgetTimeStamp";
...
int bsreadPlugin::pvGetDescription(char *pv, char *description) {
    Q_UNUSED(pv);
    qCDebug(bsread) << "bsreadPlugin:pvGetDescription";
...
int bsreadPlugin::pvClearEvent(void * ptr) {
    Q_UNUSED(ptr);
    qCDebug(bsread) << "bsreadPlugin:pvClearEvent";
    return true;
}

int bsreadPlugin::pvAddEvent(void * ptr) {
    Q_UNUSED(ptr);
    qCDebug(bsread) << "bsreadPlugin:pvAddEvent";
    return true;
}
...
int bsreadPlugin::pvReconnect(knobData *kData) {
     Q_UNUSED(kData);
    qCDebug(bsread) << "bsreadPlugin:pvReconnect";
    return true;
}

int bsreadPlugin::pvDisconnect(knobData *kData) {
    Q_UNUSED(kData);
    qCDebug(bsread) << "bsreadPlugin:pvDisconnect";
    return true;
}

// flush any io
int bsreadPlugin::FlushIO() {
    qCDebug(bsread) << "bsreadPlugin:FlushIO";
    return true;
}

// termination
int bsreadPlugin::TerminateIO() {
    qCDebug(bsread) << "bsreadPlugin:TerminateIO";
    timerValues->stop();
    timer->stop();
    return true;
}
...
void bsreadPlugin::closeEvent(){
    qCDebug(bsread) << "bsreadPlugin:closeEvent ";
    emit closeSignal();
    if (Dispatcher){
        qCDebug(bsread) << "start Dispatcher->setTerminate(); ";
        Dispatcher->initiateShutdown();
       qCDebug(bsread) << "END Dispatcher->setTerminate(); ";
    }
    if (DispatcherThread){
        qCDebug(bsread) << "start DispatcherThread ";
        DispatcherThread->exit();
        DispatcherThread->wait();
        qCDebug(bsread) << "end DispatcherThread ";
    }
