...
Q_LOGGING_CATEGORY(fileOpenWindow, "caqtdm.viewer.fileopenwindow");
...
void FileOpenWindow::onApplicationStateChange(Qt::ApplicationState state)
{
    ...
    switch (state) {
         case Qt::ApplicationSuspended:
             qCDebug(fileOpenWindow) << "application state changed to suspended";
             break;
         case Qt::ApplicationHidden:
             qCDebug(fileOpenWindow) << "application state changed to hidden";
             break;
         case Qt::ApplicationInactive:
             qCDebug(fileOpenWindow) << "application state changed to inactive";
             ...
             if (mutexKnobData != (MutexKnobData *) Q_NULLPTR) {
                 for (int i=0; i < mutexKnobData->GetMutexKnobDataSize(); i++) {
                     ...
                     if(kPtr->index != -1)  {
                       qCDebug(fileOpenWindow) << "should disconnect" << kPtr->pv;
         ...
         case Qt::ApplicationActive:
             qCDebug(fileOpenWindow) << "application state changed to active";
             pendio = false;
              if (mutexKnobData != (MutexKnobData *) Q_NULLPTR) {
                  for (int i=0; i < mutexKnobData->GetMutexKnobDataSize(); i++) {
                      knobData *kPtr = mutexKnobData->GetMutexKnobDataPtr(i);
                               ...
                               QMap<QString, QString> options): QMainWindow(parent)
{
...
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
    qCDebug(fileOpenWindow) <<  "caQtDM -- desktop size:" << qApp->desktop()->size();
#else
    qCDebug(fileOpenWindow) <<  "caQtDM -- desktop size:" <<  QGuiApplication::primaryScreen()->size();
#endif
    ...
    qCInfo(fileOpenWindow) << "caQtDM -- shared memory key" << uniqueKey;
    ...
    if (memoryAttached) {
        _isRunning = true;
        if(attach) {
            qCInfo(fileOpenWindow) << "caQtDM -- another instance of caQtDM detected with size"  << sharedMemory.size() << "==> attach to it (" << uniqueKey <<")" ;
            ...
            qCInfo(fileOpenWindow) << "send a message with file, macro and geometry to it and exit "<< message;
            sendMessage(message);
            sharedMemory.detach();
            qApp->exit(0);  // does not work here
            exit(0);
        } else {
            qCWarning(fileOpenWindow) << "caQtDM -- another instance of caQtDM detected, but no attach specified ==> standalone";
        }
    // memory to be created
        ...
        if (!sharedMemory.create(BlopSize * RingSize + 2 * sizeof(uint))) {
            qCCritical(fileOpenWindow) << "caQtDM -- Unable to create shared memory:" << sharedMemory.errorString();
        } else {
            int size =  BlopSize * RingSize + 2 * sizeof(uint);
            QByteArray byteArray(size, '\0');
            qCInfo(fileOpenWindow) << "caQtDM -- created shared memory with" << BlopSize * RingSize + 2 * sizeof(uint) << "bytes";
            sharedMemory.lock();
    ...
    if (!HmiSharedEventBus::instance().setup()) {
        qCCritical(fileOpenWindow) << "Failed to set up HmiSharedEventBus. Unable to receive or send events using this instance (pid:" << QCoreApplication::applicationPid() << ")";
    } else {
    ...
    if (!HmiSharedConfigListManager::instance().setup()) {
        qCCritical(fileOpenWindow) << "Failed to set up HmiSharedConfigListManager. Unable to view configured hmiConfigs of other processes (pid:" << QCoreApplication::applicationPid() << ")";
    } else {
        ...
        connect(heartBeatTimer, &QTimer::timeout, this, [](){
            ...
            qCDebug(fileOpenWindow) << "HeartBeatTimer tick" << time;
            {
...
//************************************************************************************************************************************************
    ...
    qCDebug(fileOpenWindow) <<  "desktop size in millimer" << qApp->desktop()->widthMM() << qApp->desktop()->heightMM();
#else
    QSize desktopSize = QGuiApplication::primaryScreen()->size();
    qCDebug(fileOpenWindow) <<  "desktop size in millimer" << QGuiApplication::primaryScreen()->physicalSize().width() << QGuiApplication::primaryScreen()->physicalSize().height();
#endif
 ...
 again:
    ...
    qCDebug(fileOpenWindow) << stdpathdoc;
    ...
    qCDebug(fileOpenWindow) << "urls" << urls;
...
void FileOpenWindow::timerEvent(QTimerEvent *event)
{
    ...
    if(printandexit) {
            ...
            if(this->findChildren<CaQtDM_Lib *>().count() == 1) {
                ...
                if(countDisplayed > 0 && countDisplayed == countPV) {
                    ...
                    if(printIt > 2) {
                        ...
                        qCInfo(fileOpenWindow) << "caQtDM -- file has been printed to caQtDM.ps";
                        qApp->exit(1);
                        exit(1);
                    }
                }
        ...
        if(timeout++ > 4) {    // seems we did not get everything
            CaQtDM_Lib * widget = this->findChild<CaQtDM_Lib *>();
            widget->printPS("caQtDM.ps");
            qCWarning(fileOpenWindow) << "caQtDM -- file has been printed to caQtDM.ps, probably with errors";
            qApp->exit(1);
            exit(1);
        }
    }
    ...
    if(!var.isNull()) {
        ...
        if (savetoimage){
                ...
                if(this->findChildren<CaQtDM_Lib *>().count() == 1) {
                    ...
                    if(countDisplayed > 0 && countDisplayed == countPV) {
                        ...
                        if(printIt > 2) {
                            ...
                            qCInfo(fileOpenWindow) << "caQtDM -- file has been printed to "<< name;
                            qApp->exit(1);
                            exit(1);
                        }
                    }
            ...
            if(timeout++ > 4) {    // seems we did not get everything
                CaQtDM_Lib * widget = this->findChild<CaQtDM_Lib *>();
                widget->save_graphics(name);
                qCWarning(fileOpenWindow) << "caQtDM -- file has been printed to " << name << ", probably with errors";
                qApp->exit(1);
                exit(1);
            }
    ...
    if(!reloadList.isEmpty()) {
        foreach( Row row, reloadList ) {
           qCInfo(fileOpenWindow) << "caQtDM -- reload file" << row.file << "with macro" << row.macro << "can be resized=" << row.resize;
...
void FileOpenWindow::setDirectUpdateTypeOnRestart(const QDateTime reloadTime){
    ...
    qCInfo(fileOpenWindow) << "UpdateType reset to direct";
    if(messageWindow != (MessageWindow *) Q_NULLPTR) {
                                    ...
                                    const bool &printexit, const bool &moveit, const bool &centerwindow)
{
    ...
    qCDebug(fileOpenWindow) << "IOS screen" << screengeo<< availscreengeo;
    QSize winsize=mainWindow->size();
    winsize.setHeight( winsize.rheight() -availscreengeo.top());
    mainWindow->resize(winsize);

#endif
    qCDebug(fileOpenWindow) << "allocate" << fileS.size()+macroS.size()+100;
...
{
    qCDebug(fileOpenWindow) << "callback open new file" << inputFile << "with macro string" << macroString;
    ...
    if (!check_extension) {
        while (counter<extensions){
            ...
            qCDebug(fileOpenWindow) << "check " << FileName << valid_extensions[counter] ;
    ...
    qCDebug(fileOpenWindow) << "try to open file" << FileName;
    ...
    foreach(QWidget* widget, all) {
        if(QMainWindow* w = qobject_cast<QMainWindow *>(widget)) {
            ...
            qCDebug(fileOpenWindow) << "existing filename with macro =" <<  w->windowTitle() << "=" <<  w->property("fileString") << macroString;
            if(!fileName.isNull()) {
            ...
            qCDebug(fileOpenWindow) << "title=" << title << " windowproperty=" << WindowProperty;
...
// in case of x windows, we will pop the window really up
    ...
    if(fileNameFound.isNull()) {
        ...
        qCDebug(fileOpenWindow) << "file" << fileNameFound << "will be loaded" << "macro=" << macroString;
        QPoint position(0,0);
        ...
        foreach(QWidget* widget, all) {
            if(QMainWindow* w = qobject_cast<QMainWindow *>(widget)) {
                ...
                if(processWindow *w1 =  qobject_cast<processWindow *>(widget)) {
                  ...
                  qCDebug(fileOpenWindow) << "close window" << w;
                  w->close();
                }
#else
                w->close();
...
void FileOpenWindow::reload(QWidget *w)
{
    ...
    if(!fileName.isNull()) {
        ...
        if (this->ui.directAction->isChecked()) {
            ...
            qCWarning(fileOpenWindow) << "Setting UpdateType to timed for 10 Seconds, can't start up with UpdateType = direct";
            if(messageWindow != (MessageWindow *) Q_NULLPTR) {
...
void FileOpenWindow::checkForMessage()
{
    ...
    sharedMemory.unlock();
    if(element.blop[0] == '\0') {
        qCDebug(fileOpenWindow) << "queue was empty, so do nothing";
        return;  // no message, quit
    }
    QString message = QString::fromUtf8(element.blop); // get and split message
    QStringList vars = message.split(";");

    qCDebug(fileOpenWindow) << "received message=" << message;
    qCDebug(fileOpenWindow) << "vars" << vars.count() <<  vars;
...
void FileOpenWindow::parse_and_set_Geometry(QMainWindow *widget, QString parsestring) {
    ...
    qCDebug(fileOpenWindow) << "set window" << w << "to" << x << y << w << h;
...
bool FileOpenWindow::event(QEvent *e)
{
    if (e->type() == QEvent::Show) {
        qCDebug(fileOpenWindow)<<"QEvent::Show!";
